...
- Wenn die gebundene Python-Helper-API unter `Host/Python/**` oder `UiEditor/Templates/ui_python_client/**` geaendert oder erweitert wird, muessen die Dokumentationen `Host/Python/Integration/ui-python-client-commands.md` und `UiEditor/Templates/ui_python_client/COMMANDS.md` im selben Change mit aktualisiert werden.
- `Host/Python/Integration/ui-python-client-commands.md` gilt als Source of Truth fuer vordefinierte Python-Client-Kommandos.
- Wenn Python-Bridge-Verhalten, generierte Python-Ordnerinhalte, Template-Workflow oder Python-Interaktionsargumente geaendert werden, muessen `Host/Python/Integration/python-system-overview.md` und `UiEditor/Templates/PYTHON_SYSTEM.md` im selben Change mit aktualisiert werden.
- Bei Arbeiten an Python-Templates, Python-Umgebungen oder generierten Python-Skripten sollen `Host/Python/Integration/python-system-overview.md` und `Host/Python/Integration/ui-python-client-commands.md` zuerst konsultiert werden.
- Wenn Widget-Code unter `UiEditor/Widgets/**` geaendert wird, muss die passende Widget-Dokumentation unter `AutomationExplorer/docs/widgets/` im selben Change mit aktualisiert werden. Es gilt eine Markdown-Datei pro Widget-Typ, und der Dateiname soll dem persistierten Widget-`Type` entsprechen, damit die Dokumentation spaeter in der Anwendung geladen werden kann.
- Wenn Widget-Code unter `UiEditor/Widgets/**` geaendert wird, muss die passende Help-Datei unter `AutomationExplorer/docs/widgets/help/` im selben Change mit aktualisiert werden. Es gilt eine Markdown-Datei pro Widget-Typ mit dem Namensmuster `<Type>.help.md`, damit die Hilfe spaeter in einem Help-Fenster geladen werden kann.
...
- Vor Arbeiten an Python-Dateien soll `Host/Python/Integration/python-system-overview.md` gelesen werden, um Runtime- und Ordnerstruktur zu verstehen.
- Vor Arbeiten an Python-Dateien soll `Host/Python/Integration/ui-python-client-commands.md` gelesen werden, um die unterstuetzte Helper-API zu kennen.
