# Aufgabe: Folder-Name nicht mehr in YAML speichern, sondern aus Dateinamen ableiten

## Ziel

Das FolderModel soll keinen redundanten `Name`-Eintrag mehr in der YAML-Datei enthalten.

Stattdessen gilt kuenftig:

- der technische Folder-Name wird immer aus dem Dateinamen ohne `.yaml` abgeleitet
- die YAML-Datei enthaelt keinen `Name` mehr
- es gibt damit nur noch eine Quelle fuer die technische Identitaet einer Folder
...
Die technische Identitaet einer Folder soll ausschliesslich aus dem Dateinamen kommen.

Konkret:

- `Name` wird aus dem YAML-Modell entfernt
- beim Laden wird `Path.GetFileNameWithoutExtension(...)` als Folder-Name verwendet
...
Beim Laden einer Folder-Datei gilt:

1. Dateipfad der YAML bestimmen
2. Dateinamen ohne Erweiterung lesen
3. diesen Wert als technischen Folder-Namen setzen
4. YAML-Inhalt ohne `Name` deserialisieren

Beispiel:

- Datei: `Folders/Overview.yaml`
- resultierender Folder-Name: `Overview`
...
Wenn eine Folder technisch umbenannt werden soll, erfolgt das ueber den Dateinamen.

Beispiel:

- vorher: `Overview.yaml`
- nachher: `MachineOverview.yaml`
- technischer Folder-Name nach Reload: `MachineOverview`
...
Alle internen Referenzen, die sich auf die technische Folder-Identitaet beziehen, muessen mit diesem abgeleiteten Namen arbeiten.
...
- abgeleiteten Namen in das interne FolderModel uebernehmen
...
- technischen Folder-Namen immer aus dem Dateinamen ableiten
