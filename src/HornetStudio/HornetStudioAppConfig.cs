...
public sealed class HornetStudioAppConfig
{
    public string? StartLayout { get; set; }
    public string? DefaultTheme { get; set; }

    public static HornetStudioAppConfig Load(string path)
    {
        try
        {
            if (!File.Exists(path))
            {
                return new HornetStudioAppConfig();
            }

            var config = new HornetStudioAppConfig();
    ...
    public void Save(string path)
    {
        try
        {
            ...
            writer.WriteLine("# HornetStudio application configuration");
