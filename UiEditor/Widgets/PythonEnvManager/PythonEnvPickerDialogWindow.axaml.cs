...
public sealed class PythonEnvTemplateItem
{
...
public sealed partial class PythonEnvPickerDialogWindow : Window, INotifyPropertyChanged
{
    private readonly MainWindowViewModel? _viewModel;
    private readonly HashSet<string> _existingNames;
    private MainWindowViewModel? _subscribedViewModel;
    private string _envName = string.Empty;
    private PythonEnvTemplateItem? _selectedTemplate;
    ...
    public PythonEnvPickerDialogWindow(MainWindowViewModel? viewModel, IEnumerable<string>? existingNames = null)
    {
    ...
    public ObservableCollection<PythonEnvTemplateItem> Templates { get; }
    ...
    public PythonEnvTemplateItem? SelectedTemplate
    {
    ...
    private void LoadTemplates()
    {
        ...
        {
            Templates.Add(new PythonEnvTemplateItem
            {
                DisplayName = "New",
                TemplatePath = null
            });
        ...
        try
        {
            ...
            if (Directory.Exists(templatesDir))
            {
                foreach (var file in Directory.EnumerateFiles(templatesDir, "*.py"))
                {
                    ...
                    var displayName = string.Equals(name, "PythonEnvNew", StringComparison.OrdinalIgnoreCase)
                        ? "New"
                        : name;
    ...
    private void UpdateSummary()
    {
        ...
        {
            Summary = "Enter a name for the environment.";
            return;
        }

        var templateName = SelectedTemplate?.DisplayName ?? "New";
        Summary = $"Environment '{EnvName}' will be created using template '{templateName}'.";
    }
    ...
    private void UpdateTemplatePreview()
    {
        ...
        TemplatePreview = "# Python environment script" + Environment.NewLine + Environment.NewLine
    ...
    private void OnSaveClicked(object? sender, RoutedEventArgs e)
    {
        ...
        {
            ErrorMessage = $"An environment named '{EnvName}' already exists.";
            return;
        }

        try
        {
            var envDefinition = _viewModel.CreatePythonEnvironmentFromTemplate(EnvName, SelectedTemplate.TemplatePath);
